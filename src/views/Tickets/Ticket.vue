...
<div class='ticket' @click="ticketMenuShow=true">
    ...
    <div class="tabs clear-float">
        <div :class="['tabs-item',{'tabs-item-active':tabsActive==item.text} ]" v-for="(item,index) in tabsArr" :key="item.id+index" @click="handlerTabs(item.text)"S>
    ...
    <div class="ticket-tabs">
        <div :class="['ticket-tabs-item', 'ticket-button',{'ticket-tabs-active':ticketActive==item.text}]"  v-for="(item,index) in ticketArr" :key="item.id+index" @click="handerTicket(item.text)">
            {{item.text}}
        </div>
    </div>
</div>
...
components: {
    allTickets:{
        template:`
            <section>全部外卖券</section>
        `
    },
    shopExchange:{
        template:`
            <section>商品兑换券</section>
        `
    }
},
data() {
